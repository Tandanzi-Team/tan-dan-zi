...
// StackedBarChart 컴포넌트는 막대 차트를 그리는 React 함수형 컴포넌트
// 이 컴포넌트는 data와 colors를 props로 받아옴
const StackedBarChart: React.FC<LegendComponentProps> = ({ data = [], colors, index }) => {
  ...
  useEffect(() => {
    ...
    const margin = { top: 50, right: 25, bottom: 80, left: 25 };
    const width = 100;
    ...
    const stackData = data.map((d, i) => {
      const gap = 2; // 여백 값을 설정 이 값을 조절하여 간격의 크기를 변경할 수 있음
    ...
    svg.selectAll(".chart-title")
      .data([titles[index]]) // index는 차트의 인덱스 번호
      .enter()
      .append("text")
